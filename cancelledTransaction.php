<?php 
// Fetching cancelled transactions
session_start();
...
if(x_validatesession("TIMBOSS_CRYPTBUY_ID")){
	...
	if(x_count("transaction","status='2' AND user_id='$userid' LIMIT 1") > 0){
		?>
		<p class="labelme">All cancelled transactions</p>
		<ul style="font-size:9pt;" class="list-group"><?php
		foreach(x_select("id,amount,tranx_id,wallet_type,type,date_time,time_stamp","transaction","status='2' AND user_id='$userid'","10","id desc") as $details){
		...
		x_print("<ul class='list-group'><li class='list-group-item'><p class='text-center'><i style='color:lightgray;font-size:50pt;' class='fa fa-trash mt-2'></i><br/><br/>No cancelled Transaction<p></li></ul>");
	}
}
?>
