...
if(x_validatemethod("POST")){
	...
	if(x_count("transaction","status = '1' OR status='2'") > 0){
		...
		<table class="table" id="table_id">
			...
			<tbody>
		<?php
		foreach(x_select("0","transaction","status = '1' OR status='2'","500","id desc") as $key){
			...
			?>
				...
				<?php
					if($status == "1"){
						?>
						  <span class="badge bg-p color-w"> Approved</span>
						<?php
					}
					
				    if($status == "2"){
						?>
							<span class="badge  bg-g color-w">Rejected</span>
						<?php
					}
